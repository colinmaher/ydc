            ...
            for (;;)
            {
                  try
                  {
                        ...
                        case tsymbol::SCANEOF:
                              ...
                              break;
                        case tsymbol::NUMBER:
                              // cout << lexeme.lexinfo << endl;
                              operand_stack.push(bigint(lexeme.lexinfo));
                              break;
                        case tsymbol::OPERATOR:
                        {
                              fn_hash::const_iterator fn = do_functions.find(lexeme.lexinfo);
                              if (fn == do_functions.end())
                              {
                                    throw ydc_exn(octal(lexeme.lexinfo[0]) + " is unimplemented");
                              }
